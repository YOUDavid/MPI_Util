...
void *wrapper_presidue_Local(void *data){
    ...
    while (TRUE){
        ...
        if (sg_i >= sg_len_pl_tdi){
            ...
            delete_np_matrix(&C);
            delete_np_matrix(&D);
            flogf(stderr, "mpi_lpresidue: dest=%d was distributed %d tasks\n", 0, total_count);
            pthread_exit(NULL); 
        }        
    }
} 
...
void mpi_lpresidue(int len_pl_tdi, 
	...
	flogf(stderr, "mpi_lpresidue: %f seconds for all Bcasts to finish on processor %s, rank %d out of %d worlds.\n", 
	...
	if (WORLD_RANK == 0){ 
		...
		flogf(stderr, "mpi_lpresidue: total %d tasks\n", sg_len_pl_tdi);
		...
		for (i = 0; i < len_pl_tdi; ++i){
			MPI_Bcast(T_di[i], m_tdi[i] * n_tdi[i], MPI_DOUBLE, 0, MPI_COMM_WORLD);//, tdi_status + i);
			MPI_Bcast(result[i], m_result[i] * n_result[i], MPI_DOUBLE, 0, MPI_COMM_WORLD);//, result_status + i);
		}
		
		
		for (i = 0; i < len_s_f; ++i){
			MPI_Bcast(S_matrix[i], m_sm[i] * n_sm[i], MPI_DOUBLE, 0, MPI_COMM_WORLD);//, sm_status + i);
			MPI_Bcast(F_matrix[i], m_fm[i] * n_fm[i], MPI_DOUBLE, 0, MPI_COMM_WORLD);//, fm_status + i);
		}
		...
		flogf(stderr, "mpi_lpresidue: %f seconds for cthreads to finish on root.\n", omp_get_wtime() - stime);
		...
		for (i = 0; i < len_pl_tdi; ++i){
			...
			MPI_Bcast(sg_T_di[i], temp_size, MPI_DOUBLE, 0, MPI_COMM_WORLD); //, tdi_status + i);
			
			temp_size = sg_m_result[i] * sg_n_result[i];
			sg_result[i] = malloc(sizeof(double) * temp_size);
			MPI_Bcast(sg_result[i], temp_size, MPI_DOUBLE, 0, MPI_COMM_WORLD); //, result_status + i);			
		}
		
		for (i = 0; i < len_s_f; ++i){
			temp_size = sg_m_sm[i] * sg_n_sm[i];
			sg_S_matrix[i] = malloc(sizeof(double) * temp_size);
			MPI_Bcast(sg_S_matrix[i], temp_size, MPI_DOUBLE, 0, MPI_COMM_WORLD); //, sm_status + i);
			
			temp_size = sg_m_fm[i] * sg_n_fm[i];
			sg_F_matrix[i] = malloc(sizeof(double) * temp_size);
			MPI_Bcast(sg_F_matrix[i], temp_size, MPI_DOUBLE, 0, MPI_COMM_WORLD); //, fm_status + i);
		}
        ...
        sem_init(&t_lock, 0, 0); //thread lock
    ...
    sem_destroy(&i_lock);
	flogf(stderr, "mpi_lpresidue: %f seconds taken from processor %s, rank %d out of %d worlds.\n", 
        omp_get_wtime() - stime, PROCESSOR_NAME, WORLD_RANK, WORLD_SIZE);
}
...
void pth_lpresidue(int len_pl_tdi, 
    ...
    sem_destroy(&i_lock);
    flogf(stderr, "pth_lpresidue: %f seconds taken.\n", omp_get_wtime() - stime);
}
